...
def calculate():
    try:
        ...
        efficiencies = {
            'fireplace': {'efficiency': 0.30, 'type': 'ancient'},
            'hypocaust': {'efficiency': 0.45, 'type': 'ancient'},
            'ondol': {'efficiency': 0.50, 'type': 'ancient'},
            'pech': {'efficiency': 0.65, 'type': 'ancient'},
            'kang': {'efficiency': 0.40, 'type': 'ancient'},
            'brazier': {'efficiency': 0.25, 'type': 'ancient'},
            'radiator': {'efficiency': 0.85, 'type': 'modern'},
            'underfloor': {'efficiency': 0.95, 'type': 'modern'}
        }
        ...
        if heating_method not in efficiencies:
            return jsonify({'error': f"Invalid heating method: {heating_method}"}), 400
            
        if insulation not in insulation_factors:
            return jsonify({'error': f"Invalid insulation type: {insulation}"}), 400
        ...
        costs = calculate_heating_costs(heating_method, room_area, efficiency_percentage)
        result.update(costs)
        
        # Add environmental impact calculations
        environmental = calculate_environmental_impact(heating_method, room_area, efficiency_percentage)
